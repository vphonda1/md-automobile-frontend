...
export default function CustomerManagement() {
  ...
  const filtered = customers.filter(c => {
    ...
    return [
      ...
      c.mobileNo, c.mobile, c.phone,
      c.chassisNo, c.vehicleModel, c.batteryNumber,
      c.fatherName, c.aadhar
    ...
    <div className="p-4 max-w-7xl mx-auto fade-in" style={{ paddingBottom: 100 }}>
      ...
      <div className="card mb-4">
        <div className="flex items-center gap-2">
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search name, mobile, chassis, battery, father, aadhar..." />
        </div>
      </div>
      ...
      {loading ? <div className="text-center py-8 text-slate-400">Loading...</div> :
       ...
       <div className="space-y-2">
         {paginated.map(c => {
           const name = c.customerName || c.name || 'Unnamed';
           const mobile = c.mobileNo || c.mobile || c.phone || '';
           return (
           <div key={c._id} className="card card-hover">
             <div className="flex items-start justify-between gap-2">
               <Link to={`/customers/${c._id}`} className="flex-1 min-w-0">
                 ...
                 <div className="text-sm text-slate-300">{c.vehicleModel || '—'} {c.color && `• ${c.color}`}</div>
                 <div className="text-xs text-slate-400 mt-1">
                   {mobile && `📞 ${mobile}`}
                   {c.chassisNo && ` • 🔧 ${c.chassisNo}`}
                 </div>
                 {c.batteryNumber && <div className="text-xs text-yellow-400 mt-1">🔋 Battery: {c.batteryNumber}</div>}
                 {(c.invoiceDate || c.soldAt) && <div className="text-xs text-slate-500 mt-1">Purchased: {formatDate(c.invoiceDate || c.soldAt)}</div>}
               </Link>
               <div className="flex flex-col gap-1">
                 {mobile && <>
                   <button onClick={() => openWhatsApp(mobile, `नमस्ते ${name}, MD Automobile से।`)} className="p-2 rounded bg-green-700"><MessageCircle size={14} /></button>
                   <a href={`tel:${mobile}`} className="p-2 rounded bg-blue-700"><Phone size={14} /></a>
                 </>}
      ...
      {confirmDelete && (
        <AdminPasswordModal
          ...
          itemName={`${confirmDelete.customerName || confirmDelete.name || 'Unknown'} (${confirmDelete.mobileNo || confirmDelete.mobile || '—'})`}
          onConfirm={doDelete}
