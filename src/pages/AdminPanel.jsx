...
export default function AdminPanel() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [cRaw, vRaw, iRaw, jRaw, sRaw, st] = await Promise.all([
      ...
      const c = Array.isArray(cRaw) ? cRaw : (cRaw?.customers || []);
      const v = Array.isArray(vRaw) ? vRaw : (vRaw?.vehicles || []);
      const i = Array.isArray(iRaw) ? iRaw : (iRaw?.invoices || []);
      const j = Array.isArray(jRaw) ? jRaw : (jRaw?.jobcards || []);
      const s = Array.isArray(sRaw) ? sRaw : (sRaw?.staff || []);
      setStats({
