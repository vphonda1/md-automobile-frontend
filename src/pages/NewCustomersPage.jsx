...
export default function NewCustomersPage() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      // Send all field name variants for compatibility with imported data
      const payload = {
        ...form,
        name: form.customerName,            // duplicate
        mobile: form.mobileNo,              // duplicate
        phone: form.mobileNo,               // duplicate
        mobileNo: form.mobileNo,
        aadharNo: form.aadhar,
        panNo: form.pan,
        city: form.city,
        district: form.city,                // duplicate for backward compat
        chassisNo: form.chassisNo,
        vehicleModel: form.vehicleModel,
        model: form.vehicleModel,           // duplicate
        // Standard amount fields
        totalAmount: Number(form.finalPaidPrice) || Number(form.onRoadPrice) || 0,
        saleAmount: Number(form.finalPaidPrice) || Number(form.onRoadPrice) || 0,
        effectivePrice: Number(form.finalPaidPrice) || Number(form.onRoadPrice) || 0,
        netAmount: Number(form.finalPaidPrice) || Number(form.onRoadPrice) || 0,
        // Source tracking
        source: form.source || 'manual'
      };
      const result = await api.post('/api/customers', payload);
      alert('✅ Customer saved!');
      navigate(`/customers/${result._id || result.customerId || result.id}`);
    } catch (err) {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto fade-in" style={{ paddingBottom: 120 }}>
      ...
      <div className="space-y-4">
        ...
        <div className="card">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <Field label="Vehicle Model" value={form.vehicleModel} onChange={(v) => update('vehicleModel', v.toUpperCase())} placeholder="ASVA, LUCKA, RUBIE, CHERRY..." />
            <Field label="Color" value={form.color} onChange={(v) => update('color', v.toUpperCase())} placeholder="BLACK, RED, WHITE, GREY" />
