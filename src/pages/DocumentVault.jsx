// ═══════════════════════════════════════════════════════════════════════════
// 🆕🆕🆕 DocumentVault.jsx — VERSION-2026-05-29-FINAL-TEXTFIRST 🆕🆕🆕
// ═══════════════════════════════════════════════════════════════════════════
...
const DOC_TYPES = [
  // === Identity ===
  { key: 'aadhar',         label: 'Aadhar Card',               icon: '🪪', hasExpiry: false, category: 'Identity' },
  { key: 'pan',            label: 'PAN Card',                  icon: '💳', hasExpiry: false, category: 'Identity' },
  { key: 'customer_photo', label: 'Customer Photo',            icon: '👤', hasExpiry: false, category: 'Identity' },
  { key: 'signature',      label: 'Customer Signature',        icon: '✍️', hasExpiry: false, category: 'Identity' },
  { key: 'bank_passbook',  label: 'Bank Passbook',             icon: '🏦', hasExpiry: false, category: 'Identity' },
  // === Vehicle Documents ===
  { key: 'vp_tax_invoice', label: 'VP Tax Invoice',            icon: '🧾', hasExpiry: false, category: 'Vehicle' },
  { key: 'challan',        label: 'Challan / MRC',             icon: '📜', hasExpiry: false, category: 'Vehicle' },
  { key: 'test_certificate', label: 'Test Certificate',        icon: '📋', hasExpiry: false, category: 'Vehicle' },
  { key: 'chassis_trace',  label: 'Chassis Trace (Engine+Chassis No)', icon: '📋', hasExpiry: false, category: 'Vehicle' },
  { key: 'chassis_photo',  label: 'Chassis Photo',             icon: '🔢', hasExpiry: false, category: 'Vehicle' },
  { key: 'delivery_photo', label: 'Delivery Photo',            icon: '📸', hasExpiry: false, category: 'Vehicle' },
  { key: 'old_rc',         label: 'Old RC Card',               icon: '🪪', hasExpiry: false, category: 'Vehicle' },
  { key: 'old_noc',        label: 'Old Bike NOC',              icon: '📑', hasExpiry: false, category: 'Vehicle' },
  // === EV Parts (NEW - MD Automobile specific) ===
  { key: 'motor_photo',    label: '⚡ Motor Number Photo',      icon: '⚡', hasExpiry: false, category: 'EV Parts' },
  { key: 'charger_photo',  label: '🔌 Charger Number Photo',    icon: '🔌', hasExpiry: false, category: 'EV Parts' },
  { key: 'controller_photo', label: '🎛️ Controller Number Photo', icon: '🎛️', hasExpiry: false, category: 'EV Parts' },
  { key: 'battery_1',      label: '🔋 Battery 1 Photo',         icon: '🔋', hasExpiry: false, category: 'EV Parts' },
  { key: 'battery_2',      label: '🔋 Battery 2 Photo',         icon: '🔋', hasExpiry: false, category: 'EV Parts' },
  { key: 'battery_3',      label: '🔋 Battery 3 Photo',         icon: '🔋', hasExpiry: false, category: 'EV Parts' },
  { key: 'battery_4',      label: '🔋 Battery 4 Photo',         icon: '🔋', hasExpiry: false, category: 'EV Parts' },
  { key: 'battery_5',      label: '🔋 Battery 5 Photo',         icon: '🔋', hasExpiry: false, category: 'EV Parts' },
  { key: 'battery_6',      label: '🔋 Battery 6 Photo',         icon: '🔋', hasExpiry: false, category: 'EV Parts' },
  // === RTO ===
  { key: 'rto_form',       label: 'RTO Form',                  icon: '🚗', hasExpiry: false, category: 'RTO' },
  { key: 'intimation',     label: 'Intimation',                icon: '📝', hasExpiry: false, category: 'RTO' },
  // === Other ===
  { key: 'other',          label: 'Other Document',            icon: '📁', hasExpiry: false, category: 'Other' },
];

// MD Automobile delivery requirements (basic docs for handover)
const DELIVERY_REQUIRED_KEYS = ['aadhar', 'pan', 'challan', 'chassis_trace', 'chassis_photo', 'delivery_photo', 'motor_photo', 'charger_photo', 'controller_photo', 'battery_1'];

// Backward-compat aliases (so existing WhatsApp share buttons keep working)
const INSURANCE_REQUIRED_KEYS = ['aadhar', 'pan', 'chassis_trace', 'chassis_photo'];
const RTO_REQUIRED_KEYS = ['aadhar', 'pan', 'chassis_trace', 'chassis_photo', 'rto_form', 'intimation'];
...
// ── Image → single-page PDF (individual file, NOT merged) ─────────────────────
async function imageToPDF(d) {
  ...
  let image;
  try { image = mime === 'image/png' ? await pdf.embedPng(imgBytes) : await pdf.embedJpg(imgBytes); }
  catch { try { image = await pdf.embedJpg(imgBytes); } catch { return null; } }
  ...
  const fileName = `${d.customerName}_${d.docTypeLabel}`.replace(/[\\/:*?"<>|]/g, '_').substring(0, 60);
  return new File([bytes], `${fileName}.pdf`, { type: 'application/pdf' });
}

// ── Share — हर doc अलग PDF, सब एक साथ multi-share (जैसे multi-image पहले काम करता था) ──
async function shareFilesAuto(docs, textMsg, title) {
  ...
  // ✅ STEP 1: हर doc को अलग PDF file बनाएं (images → PDF, existing PDFs → as-is)
  showInAppToast('🆕 v5', `[NEW CODE] ${valid.length} अलग-अलग PDFs बना रहे हैं — कोई merge नहीं`, 'info');
  const files = [];
  const fileLabels = [];
    ...
    try {
      if (d.fileType === 'pdf') {
        ...
        const pdfFile = dataURLtoFile(
          d.fileData,
          `${d.customerName}_${d.docTypeLabel}`.replace(/[\\/:*?"<>|]/g, '_').substring(0, 60) + '.pdf',
          'application/pdf'
        );
        if (pdfFile) { files.push(pdfFile); fileLabels.push(d.docTypeLabel); }
        ...
        // Video
        const videoFile = dataURLtoFile(d.fileData, `${d.customerName}_${d.docTypeLabel}.mp4`, 'video/mp4');
        if (videoFile) { files.push(videoFile); fileLabels.push(d.docTypeLabel); }
      }
    } catch (e) { console.warn(`Failed: ${d.docTypeLabel}`, e.message); }
  }

  if (files.length === 0) {
    alert('❌ PDFs बनाने में error। pdf-lib install है? npm install pdf-lib');
    return false;
  }

  console.log(`[Share] ${files.length} separate PDFs:`, fileLabels);

  // ✅ STEP 2: Text clipboard में copy + single multi-share (most reliable)
  // Browser limitation: navigator.share chain नहीं हो सकता gesture के बिना
  // Solution: text clipboard में रखें + सब files एक share में
  if (navigator.share) {
    // ── Text clipboard में copy करें (fallback अगर WhatsApp text drop करे) ──
    try {
      await navigator.clipboard.writeText(textMsg);
      showInAppToast('📋 Text COPIED!', 'अगर WhatsApp में text नहीं दिखे — long-press → Paste करें', 'success');
      await new Promise(r => setTimeout(r, 2500)); // user पढ़ ले
    } catch (e) {
      console.warn('Clipboard write failed:', e.message);
    }

    // ── सब files multi-share (text caption के साथ) ──
    if (navigator.canShare && navigator.canShare({ files })) {
      try {
        await navigator.share({ title, text: textMsg, files });
        showInAppToast('✅', `${files.length} PDFs + text clipboard में`, 'success');
        return true;
      } catch (e) {
    ...
    // Fallback: एक-एक करके (पहली में text caption)
    showInAppToast('⚠️', 'Multi-share fail — एक-एक करके भेज रहे हैं', 'warning');
    for (let i = 0; i < files.length; i++) {
      const cap = i === 0 ? textMsg : `📄 ${fileLabels[i]} (${i+1}/${files.length})`;
      try {
        await navigator.share({ title: files[i].name, text: cap, files: [files[i]] });
  ...
  // Desktop: download all + WhatsApp text
...
export default function DocumentVault() {
  ...
  const [form,         setForm]         = useState({
    ...
    motorNo: '', chargerNo: '', controllerNo: '', batteryNo: '',
    nomineeName: '', hypothecation: '',
    docType: 'aadhar', expiryDate: '', notes: ''
  });
  ...
  useEffect(() => {
    ...
    api.get('/api/customers')
      ...
      .catch((err) => console.warn('Customer fetch failed:', err.message));
  }, []);
  ...
  const filteredCustomers = custSearch.trim().length >= 1
    ? customers.filter(c =>
        ...
        (c.mobileNo || c.phone || c.mobile || '').includes(custSearch) ||
        (c.aadhar || '').includes(custSearch) ||
        (c.fatherName || '').toLowerCase().includes(custSearch.toLowerCase())
      ).slice(0, 10)
    : customers.slice(0, 10);  // Show top 10 by default

  const selectCustomer = async (c) => {
    const name = c.customerName || c.name || '';
    const mobile = c.mobileNo || c.phone || c.mobile || '';
    const aadhar = c.aadhar || c.aadharNo || '';
    const customerId = c._id;

    // Build initial form from customer data
    let updates = {
      customerName: name,
      customerPhone: mobile,
      aadharNo: aadhar,
      vehicleModel: c.vehicleModel || c.model || '',
      chassisNo: c.chassisNo || '',
      motorNo: c.motorNo || c.motorNumber || '',
      chargerNo: c.chargerNo || '',
      controllerNo: c.controllerNo || '',
      batteryNo: c.batteryNumber || (Array.isArray(c.batteryNumbers) ? c.batteryNumbers.join(', ') : '') || '',
      nomineeName: c.nomineeName || '',
      hypothecation: c.hypothecation || c.financerName || ''
    };

    // Try to fetch linked vehicle for fuller details
    try {
      const vehiclesRaw = await api.get('/api/vehicles').catch(() => []);
      const allVehicles = Array.isArray(vehiclesRaw) ? vehiclesRaw : (vehiclesRaw?.vehicles || []);
      const matchedVeh = allVehicles.find(v =>
        (customerId && (v.customerId === customerId || String(v.customerId) === String(customerId))) ||
        (c.chassisNo && v.chassisNo === c.chassisNo) ||
        (name && v.customerName === name)
      );
      if (matchedVeh) {
        updates.vehicleModel = matchedVeh.model || matchedVeh.vehicleModel || updates.vehicleModel;
        updates.chassisNo = matchedVeh.chassisNo || updates.chassisNo;
        updates.motorNo = matchedVeh.motorNo || updates.motorNo;
        updates.chargerNo = matchedVeh.chargerNo || updates.chargerNo;
        updates.controllerNo = matchedVeh.controllerNo || updates.controllerNo;
        if (Array.isArray(matchedVeh.batteryNumbers) && matchedVeh.batteryNumbers.length > 0) {
          updates.batteryNo = matchedVeh.batteryNumbers.join(', ');
        }
      }
    } catch (err) {
      console.warn('Vehicle autofill failed:', err.message);
    }
  ...
  const saveDoc = async (stayOpen = false) => {
    ...
    const payload = {
      ...
      motorNo: form.motorNo, chargerNo: form.chargerNo, controllerNo: form.controllerNo, batteryNo: form.batteryNo,
    ...
    try {
      ...
      if (!stayOpen) {
        ...
        setForm({ customerName:'', customerPhone:'', aadharNo:'', vehicleModel:'', chassisNo:'', motorNo:'', chargerNo:'', controllerNo:'', batteryNo:'', nomineeName:'', hypothecation:'', docType:'aadhar', expiryDate:'', notes:'' });
      } else {
        setFileData(null);
      ...
      console.error('[DocumentVault Save]', err);
      showInAppToast('❌ Save Failed', errMsg.slice(0, 80), 'error');
    }
    setSaving(false);
  };
  ...
  if (error) return (
    <div style={{ padding:20, background:'#020617', minHeight:'100vh' , paddingBottom: 100 }}>
        ...
        <div onClick={() => { setShowForm(false); setFileData(null); }}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ display:'grid', gap:10 }}>
              ...
              <div><label style={lbl}>🛵 Vehicle Model</label><input value={form.vehicleModel} onChange={e => setForm({...form, vehicleModel: e.target.value.toUpperCase()})} placeholder="ASVA, LUCKA, RUBIE, CHERRY..." style={inp}/></div>
              <div><label style={lbl}>🔧 Chassis No</label><input value={form.chassisNo} onChange={e => setForm({...form, chassisNo: e.target.value.toUpperCase()})} placeholder="MMCRBAY0H..." style={inp}/></div>
              {/* === MD Automobile EV-specific fields === */}
              <div style={{ background:'#0f172a', padding:10, borderRadius:8, border:'1px solid #16a34a' }}>
                <div style={{ color:'#16a34a', fontSize:11, fontWeight:'bold', marginBottom:6 }}>⚡ EV PARTS (auto-fill from selected customer)</div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                  <div><label style={{ ...lbl, fontSize:10 }}>⚡ Motor No</label><input value={form.motorNo} onChange={e => setForm({...form, motorNo: e.target.value.toUpperCase()})} placeholder="YHL250W-27" style={{ ...inp, fontSize:11 }}/></div>
                  <div><label style={{ ...lbl, fontSize:10 }}>🔌 Charger No</label><input value={form.chargerNo} onChange={e => setForm({...form, chargerNo: e.target.value.toUpperCase()})} placeholder="MYKLB2612..." style={{ ...inp, fontSize:11 }}/></div>
                  <div style={{ gridColumn:'1 / -1' }}><label style={{ ...lbl, fontSize:10 }}>🎛️ Controller No</label><input value={form.controllerNo} onChange={e => setForm({...form, controllerNo: e.target.value.toUpperCase()})} placeholder="MDKL/B26/1..." style={{ ...inp, fontSize:11 }}/></div>
                  <div style={{ gridColumn:'1 / -1' }}><label style={{ ...lbl, fontSize:10 }}>🔋 Battery Nos (comma-separated, up to 6)</label><input value={form.batteryNo} onChange={e => setForm({...form, batteryNo: e.target.value.toUpperCase()})} placeholder="HTP5X22JP..., HTP5K25LAZ..., ..." style={{ ...inp, fontSize:11 }}/></div>
                </div>
              </div>
              <div><label style={lbl}>📁 Document Type *</label>
                <select value={form.docType} onChange={e => setForm({...form, docType: e.target.value})} style={inp}>
                  <optgroup label="🪪 Identity">
                    {DOC_TYPES.filter(t => t.category === 'Identity').map(t => <option key={t.key} value={t.key}>{t.icon} {t.label}</option>)}
                  </optgroup>
                  <optgroup label="🛵 Vehicle Documents">
                    {DOC_TYPES.filter(t => t.category === 'Vehicle').map(t => <option key={t.key} value={t.key}>{t.icon} {t.label}</option>)}
                  </optgroup>
                  <optgroup label="⚡ EV Parts">
                    {DOC_TYPES.filter(t => t.category === 'EV Parts').map(t => <option key={t.key} value={t.key}>{t.icon} {t.label}</option>)}
                  </optgroup>
                  <optgroup label="🏛️ RTO">
                    {DOC_TYPES.filter(t => t.category === 'RTO').map(t => <option key={t.key} value={t.key}>{t.icon} {t.label}</option>)}
                  </optgroup>
                  <optgroup label="📁 Other">
                    {DOC_TYPES.filter(t => t.category === 'Other').map(t => <option key={t.key} value={t.key}>{t.icon} {t.label}</option>)}
                  </optgroup>
                </select>
              </div>
