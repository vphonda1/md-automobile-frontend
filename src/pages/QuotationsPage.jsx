...
function QuotationForm({ initial, onClose, onSaved }) {
  ...
  const update = (key, value) => {
    ...
    // 🆕 AUTO-FILL Loan Amount = Price - Down Payment (whenever price or downPayment changes)
    if (key === 'price' || key === 'downPayment' || key === 'accessories') {
      const totalCost = n(u.price) + n(u.accessories) - u.totalSubsidy;
      const autoLoan = totalCost - n(u.downPayment);
      u.loanAmount = autoLoan > 0 ? autoLoan : 0;
    }

    // Grand Total = Price + Accessories - Subsidy (या fallback: down + loan)
...
function QuotationPreview({ quotation, onClose }) {
  ...
  // ─── Derive missing/inconsistent fields from quotation data (handles old + new records) ───
  const n = (v) => Number(v) || 0;
  const _price = n(quotation.price) || n(quotation.exShowroomPrice);
  const _totalSubsidy = n(quotation.totalSubsidy) || (n(quotation.fameSubsidy) + n(quotation.stateSubsidy));
  const _accessories = n(quotation.accessories);
  const _downPayment = n(quotation.downPayment);
  const _loanAmount = n(quotation.loanAmount);

  const fromPrice = _price + _accessories - _totalSubsidy;
  const fromFinance = _downPayment + _loanAmount;
  const _grandTotal = n(quotation.grandTotal) || (fromPrice > 0 ? fromPrice : fromFinance);
  ...
  const q = {
    ...
    batteryWarranty: quotation.batteryWarranty || '1 Year',
    motorWarranty: quotation.motorWarranty || '1 Year',
    controllerWarranty: quotation.controllerWarranty || '1 Year',
    chargerWarranty: quotation.chargerWarranty || '1 Year',
  };
