...
const TERMS = [
  ...
  '4. ब्याज दर (Interest rate) बैंक के अनुसार बदल सकती है।',
...
function QuotationForm({ initial, onClose, onSaved }) {
  const [form, setForm] = useState(initial || {
    ...
    rto: 0, insurance: 0, accessories: 0,
  ...
  const update = (key, value) => {
    ...
    u.grandTotal = n(u.price) + n(u.rto) + n(u.insurance) + n(u.accessories) - u.totalSubsidy;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 p-4 overflow-y-auto" onClick={onClose}>
      <div className="card max-w-2xl mx-auto my-4" onClick={(e) => e.stopPropagation()} style={{ paddingBottom: 60 }}>
        ...
        {/* Charges */}
        <h3 className="text-sm text-yellow-400 font-bold mt-4 mb-2">💸 Other Charges</h3>
        <div className="grid md:grid-cols-2 gap-3">
          <Field label="RTO" type="number" value={form.rto} onChange={(v) => update('rto', v)} />
          <Field label="Insurance" type="number" value={form.insurance} onChange={(v) => update('insurance', v)} />
          <Field label="Accessories" type="number" value={form.accessories} onChange={(v) => update('accessories', v)} />
        </div>
      ...
      <div className="max-w-3xl mx-auto my-4 p-2" onClick={(e) => e.stopPropagation()}>
        ...
        <div ref={previewRef} style={{
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 4 }}>
            ...
            <tbody>
              <Row n={1} p="Model Name" rate={`${q.vehicleModel || ''} ${q.variant || ''} ${q.color || ''}`} amt="" />
              <Row n={2} p="Price (Ex-Showroom)" rate="" amt={formatINR(q.price || 0)} />
              <Row n={3} p="Battery Warranty" rate={q.batteryWarranty || '—'} amt="" />
              <Row n={4} p="Motor Warranty" rate={q.motorWarranty || '—'} amt="" />
              <Row n={5} p="Controller Warranty" rate={q.controllerWarranty || '—'} amt="" />
              <Row n={6} p="Charger Warranty" rate={q.chargerWarranty || '—'} amt="" />
          ...
          {/* Totals box */}
          <div style={{ marginTop: 10, marginLeft: 'auto', width: '50%' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <tbody>
                <tr><td style={{ border: '1px solid #000', padding: 4, fontWeight: 'bold' }}>RTO</td><td style={{ border: '1px solid #000', padding: 4, textAlign: 'right' }}>{formatINR(q.rto || 0)}</td></tr>
                <tr><td style={{ border: '1px solid #000', padding: 4, fontWeight: 'bold' }}>INSURANCE</td><td style={{ border: '1px solid #000', padding: 4, textAlign: 'right' }}>{formatINR(q.insurance || 0)}</td></tr>
                <tr style={{ background: '#1e3a8a', color: '#fff' }}><td style={{ border: '1px solid #000', padding: 6, fontWeight: 'bold' }}>GRAND TOTAL</td><td style={{ border: '1px solid #000', padding: 6, textAlign: 'right', fontWeight: 'bold', fontSize: 14 }}>{formatINR(q.grandTotal || 0)}</td></tr>
              </tbody>
            </table>
          </div>
