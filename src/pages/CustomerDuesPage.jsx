...
export default function CustomerDuesPage() {
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto fade-in" style={{ paddingBottom: 100 }}>
      ...
      {showForm && <AddDueForm customers={customers} onClose={() => { setShowForm(false); load(); }} />}
...
// ═══ Add New Due Form ═══
function AddDueForm({ customers, onClose }) {
  ...
  const pickCustomer = (c) => {
    ...
    setForm(f => ({ ...f, vehicleModel: c.vehicleModel || f.vehicleModel }));
  };
      ...
      <div className="card max-w-lg mx-auto my-4" onClick={(e) => e.stopPropagation()}>
        ...
        {selectedCustomer && (
          <>
            ...
            <label className="text-xs text-slate-400 mb-1 block">🛵 Vehicle Model (optional)</label>
            <input value={form.vehicleModel} onChange={(e) => setForm({...form, vehicleModel: e.target.value})} className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm text-white mb-3" />
